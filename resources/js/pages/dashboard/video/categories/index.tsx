...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    const [video_categories, setVideoCategories] = useState<
        VideoCategory[]
    >([]);
    const [videoCategoryPagination, setVideoCategoryPagination] =
    ...
    const filteredCategory = video_categories.filter((category) => {
    ...
    const fetchProductData = async (page = 1, searchQuery = "") => {
        ...
        const CategoryResponse = await axios.get(
            `/api/v1/videos-category?page=${page}&search=${searchQuery}`,
        );

        setVideoCategories(CategoryResponse.data.categories.data);
        setVideoCategoryPagination(CategoryResponse.data.categories);
        setError(null);
        } catch (error) {
    ...
    return (
        ...
        <div className="flex h-full flex-col gap-4 rounded-xl p-4 overflow-x-auto">
            ...
            <div className="text-lg font-semibold">
                Video Category ({videoCategoryPagination?.total || 0} total)
            </div>
            ...
            {/* Results info */}
            {videoCategoryPagination && (
            <div className="text-sm text-gray-600">
                Showing {video_categories.length} of{" "}
                {videoCategoryPagination.total} Video Category
            ...
            <TableBody>
                ...
                video_categories.map((category) => (
                    ...
                    <TableCell className="flex gap-1 justify-center py-2">
                        <Link
                        href={`/video/categories/${category.id}/edit`}
            ...
            {videoCategoryPagination &&
            videoCategoryPagination.last_page > 1 && (
                <Pagination
                current_page={videoCategoryPagination.current_page}
                last_page={videoCategoryPagination.last_page}
                search={nameFilter}
                />
            )}
        </div>
