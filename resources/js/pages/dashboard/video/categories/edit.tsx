...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit Category', href: '/video/categories/edit' },
];
...
interface EditProps {
  video_categories: CategoryData; // Data kategori dari server
}

export default function EditCategory({ video_categories }: EditProps) {
    const [isSubmitting, setIsSubmitting] = useState(false);

    const { data, setData, put, processing, errors } = useForm<CategoryForm>({
        category_name: video_categories?.category_name || '',
    });
    ...
    const submit = (event: React.FormEvent) => {
        ...
        put(`/video/categories/${video_categories.id}`, {
        onSuccess: () => {
    ...
    const handleReset = () => {
        setData('category_name', video_categories?.category_name || '');
    };
    ...
    <AppLayout breadcrumbs={breadcrumbs}>
        ...
        <div className="w-full mx-auto p-6">
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="text-sm text-gray-500">ID: {video_categories?.id}</div>
            </div>
